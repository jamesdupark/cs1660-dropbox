...
class User:
    ...
    def revoke_file(self, filename: str, old_recipient: str) -> None:
        ...
        # create a new file_key; for all users who still have access to the file,
        # update their file_keys
        new_file_key = crypto.HashKDF(self.base_key, filename+crypto.SecureRandom(16).decode(errors='backslashreplace'))
        enc_new_file_key, new_file_signature = asym_enc_sign(
            dataserver.Get(user+"_pub_key"), self.sign_key, new_file_key
        )

        for user in share_list:
            sharing_string = filename+"_sharing_"+self.un+"_"+user
            sharing_key = crypto.Hash(sharing_string.encode("utf-8"))[:16]
            shared_dict_loc = generate_memloc(
                sharing_key, filename+"_sharing_"+self.un+"_"+user
            )
